...
oc create serviceaccount tester-admin-user -n default
oc adm policy add-cluster-role-to-user cluster-admin system:serviceaccount:default:tester-admin-user
...
oc create serviceaccount tester-dev-user -n default
oc adm policy add-cluster-role-to-user edit system:serviceaccount:default:tester-dev-user
DEV_TOKEN=$(oc create token tester-dev-user -n default)
oc login --token "$DEV_TOKEN" --server "$K8S_CLUSTER_URL"
