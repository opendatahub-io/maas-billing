...
This directory contains a smoke test for validating the MaaS Billing API named token functionality.

## Overview

The smoke test framework provides a simple, reusable way to test the MaaS API. It consists of:
- **`common.sh`** - Common test framework with utilities for API calls, logging, and setup
- **`named-token.sh`** - Smoke test that validates named token functionality
- **`example-test.sh`** - Simple example showing how to use the framework

## Named Token Test

The `named-token.sh` test validates the complete named token workflow:
...
5. **Revocation & expiredAt**: Revokes tokens and verifies the secret is marked as expired with a timestamp
...
```bash
# Using Podman (Recommended) - auto-detect everything
cd smoke-test
./named-token.sh
```
...
```bash
# Keep secrets after test (don't delete)
...
- **status**: Current status (starts as "active")

**Security Check**: The test also verifies that the actual token value is **NOT** stored in the secret (only metadata).

#### 5. Token Revocation & expiredAt
The test verifies that when tokens are revoked:
- Secret `status` field changes from "active" to "expired"
- New `expiredAt` field is added with the timestamp of revocation (RFC3339 format)
- Both fields are properly persisted in the Kubernetes secret
...
--- Test 4: Verifying Kubernetes secret metadata ---
...
--- Test 5: Verifying expiredAt field on revocation ---
[✓] Secret status is 'active'
[INFO] expiredAt does not exist yet (expected)
[✓] Tokens revoked
[✓] Secret status is now 'expired'
[✓] expiredAt field found: 2024-11-13T12:05:30Z
[✓] expiredAt is valid RFC3339 timestamp

[INFO] Cleaning up: Deleting test secret...

==========================================
📊 Named Token Test - Summary
==========================================
Passed:   10
Failed:   0
Warnings: 0
...
```bash
kubectl get secrets -n maas-tier-free -l maas.opendatahub.io/token-secret=true
...
2. Check maas-api is running: `kubectl get pods -n maas-api`
3. Review maas-api logs: `kubectl logs -n maas-api <pod-name>`
...
#### expiredAt Not Set
If the `expiredAt` field is not appearing after revocation:
1. Check that you're running the latest maas-api image with the fix
2. Review maas-api logs during token revocation
3. Manually inspect the secret: `kubectl get secret <name> -n maas-tier-free -o yaml`

### Cleanup

The test automatically cleans up by default:
...
# Check the expiredAt field
kubectl get secret <secret-name> -n maas-tier-free -o jsonpath='{.data.expiredAt}' | base64 -d
...
The `common.sh` file provides a shared test framework with utilities for:
...
- **Secret Inspection**: Utilities to find and read Kubernetes secrets
- **Prerequisites Checking**: Validates required tools and environment variables

### Creating New Tests

To create a new test using the common framework, see `example-test.sh`:
...
- `delete_secret "namespace" "secret"` - Delete a secret
...
All scripts are Podman-first but Docker compatible:
