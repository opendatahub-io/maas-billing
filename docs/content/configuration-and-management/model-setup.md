...
!!!Warning "Removing Models from Tiers During Active Usage"
    When updating the `alpha.maas.opendatahub.io/tiers` annotation to remove a tier, be aware that active requests may be affected. See [Model Tier Access Known Issues](./model-access-known-issues.md#model-tier-access-changes-during-active-usage) for details on side effects and recommended practices.
...
- [Model Tier Access Known Issues](./model-access-known-issues.md) - Known limitations and side effects
